...
if ($_POST['ClientInd']=='All'){
    $query='select jobs.ind, JobDescription, StartDate,  ContactFirstName, ContactLastName, Company from parkerbros.jobs left outer join parkerbros.contacts
on jobs.clientind=contacts.ind where contactselect="CLIENT"  order by jobs.startdate desc;';
}else{
  $query ='select jobs.ind, JobDescription, StartDate,  ContactFirstName, ContactLastName, Company from parkerbros.jobs left outer join parkerbros.contacts
...
echo '<input type="hidden" name="query" value="ClientJobDate">';
